...
@EqualsAndHashCode(callSuper = false, exclude = {"participante", "reuniao"}) // Excluir campos de relacionamento
public class Presenca extends Auditable { // Estende Auditable
    ...
    @JoinColumn(
            ...
            foreignKey = @ForeignKey(name = "FK_PRESENCA_PARTICIPANTE")
    )
    ...
    @JoinColumn(
            name = "REUNIAO",
            referencedColumnName = "ID_REUNIAO",
            foreignKey = @ForeignKey(name = "FK_PRESENCA_REUNIAO")
    )
