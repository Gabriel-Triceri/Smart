...
public class TarefaSearchService {
    ...
    private final TarefaCrudService crudService; // Reuse recursive logic if needed or just use repo
    ...
    public List<TarefaDTO> buscarPorTexto(String termo, Map<String, Object> filtros) {
        ...
        List<Tarefa> tarefas = tarefaRepository.findAll().stream()
                .filter(t -> t.getDescricao() != null && t.getDescricao().toLowerCase().contains(termo.toLowerCase()))
        ...
        if (filtros != null && !filtros.isEmpty()) {
            if (filtros.containsKey("status")) {
                ...
                tarefas = tarefas.stream()
                        .filter(t -> t.getStatusTarefa() != null && t.getStatusTarefa().name().equals(status))
                        .collect(Collectors.toList());
            }
    ...
    public List<TarefaDTO> getTarefasDoUsuarioAtual() {
        ...
        return tarefaRepository.findAll().stream()
                .filter(t -> {
                    ...
                    if (t.getResponsavel() != null && t.getResponsavel().getId() != null
                            && t.getResponsavel().getId().equals(currentUserId)) {
                        return true;
                    }

                    // participantes (set de Pessoa)
                    if (t.getParticipantes() != null && t.getParticipantes().stream()
                            .anyMatch(p -> p != null && p.getId() != null && p.getId().equals(currentUserId))) {
                        return true;
                    }

                    // campo criadaPor como string (compatibilidade)
                    if (t.getCreatedBy() != null && t.getCreatedBy().equals(currentUserIdStr)) {
                        return true;
                    }
