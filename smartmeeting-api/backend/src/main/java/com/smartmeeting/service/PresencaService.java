...
public class PresencaService {
    ...
    public PresencaDTO registrarPresenca(Long reuniaoId, PresencaDTO dto) {
        ...
        Pessoa pessoa = pessoaRepository.findByCrachaId(dto.getCrachaId())
                .orElseThrow(() -> new ResourceNotFoundException("Pessoa com crachá " + dto.getCrachaId() + " não encontrada."));

        if (!reuniao.getParticipantes().contains(pessoa)) {
            throw new BadRequestException("Pessoa com crachá " + dto.getCrachaId() + " não é participante da reunião " + reuniaoId);
        }
