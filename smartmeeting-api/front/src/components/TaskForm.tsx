                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                                ...
                                >
                                    {principalAssignee ? (
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span className="dark:text-white">{principalAssignee.nome}</span>
                                        </div>
                                    ) : (
                                        <span className="text-gray-500 dark:text-gray-400">Selecione o responsável principal</span>
                                    )}
                                        ...
                                        {assignees.map((assignee) => (
                                            <button
                                                ...
                                                className={`w-full px-3 py-2 text-left hover:bg-gray-50 dark:hover:bg-gray-600 flex items-center space-x-2 ${
                                                    assignee.id === formData.responsavelPrincipalId ? 'bg-blue-50 dark:bg-blue-900/30' : ''
                                                }`}
                                            >
                                                <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white ${
                                    errors.prazo_tarefa ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                                }`}
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white ${
                                errors.estimadoHoras ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                            }`}
                            placeholder="Ex: 8"
                        />
                    ...
                    <div>
                        ...
                        {(formData.tags ?? []).length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-3">
                                {(formData.tags ?? []).map((tag, index) => (
                                    <span key={index} className="inline-flex items-center px-2 py-1 bg-blue-100 text-blue-800 text-sm rounded dark:bg-blue-900/30 dark:text-blue-300">
                                        {tag}
                                        <button type="button" onClick={() => handleRemoveTag(tag)} className="ml-2 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-200">
                                            <X className="w-3 h-3" />
                                        </button>
                                    </span>
                                ))}
                        ...
                        <div className="flex space-x-2">
                            ...
                            />
                            <button type="button" onClick={() => handleAddTag(newTag)} className="px-3 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors dark:bg-gray-600 dark:text-gray-300 dark:hover:bg-gray-500">
                                Adicionar
                            </button>
                        </div>
                        <div className="mt-3">
                            <div className="text-xs text-gray-600 dark:text-gray-400 mb-2">Sugestões:</div>
                            <div className="flex flex-wrap gap-1">
                                {availableTags.filter(tag => !(formData.tags ?? []).includes(tag)).slice(0, 8).map(tag => (
                                    <button key={tag} type="button" onClick={() => handleAddTag(tag)} className="px-2 py-1 bg-gray-100 text-gray-700 text-sm rounded hover:bg-gray-200 transition-colors dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
                                        {tag}
                                    </button>
                                ))}
                            </div>
                            ...
                            {['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899'].map(color => (
                                <button
                                    ...
                                    className={`w-8 h-8 rounded-full border-2 ${
                                        formData.cor === color ? 'border-gray-800 dark:border-white' : 'border-gray-300 dark:border-gray-600'
                                    }`}
                    ...
                    <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                        <button
                            type="button"
                            onClick={onClose}
                            className="px-4 py-2 text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700"
                        >
                            Cancelar
                        </button>
                        <button
