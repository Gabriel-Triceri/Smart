...
export function UserMenu({ className = '' }: UserMenuProps) {
    ...
    const [showProfile, setShowProfile] = useState(false);   // FIX #8: estado para o modal de perfil
    ...
    return (
        <>
            <div ref={menuRef} className={`relative ${className}`}>
                ...
                {isOpen && (
                    <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-mono-800 rounded-xl shadow-lg border border-mono-200 dark:border-mono-700 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                        ...
                        {/* Opções do menu */}
                        <div className="py-1">
                            {/* FIX #8: agora abre o ProfileModal em vez de apenas fechar */}
                            <button
                                onClick={() => {
                                    setIsOpen(false);
                                    setShowProfile(true);
            ...
            {/* FIX #8: Modal de perfil renderizado fora do dropdown */}
