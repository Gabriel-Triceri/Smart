...
export const Calendar: React.FC<CalendarProps> = ({
  ...
  const renderMonthView = () => {
    ...
    const lastDay = new Date(currentView.dataReferencia.getFullYear(), currentView.dataReferencia.getMonth() + 1, 0);
          ...
          <div className="p-2">
            {Array.from({ length: 24 }, (_, hour) => {
              const hourReunioes = dayReunioes.filter(r => {
                const reuniaoHour = parseInt(r.horaInicio.split(':')[0]);
                  ...
                  {hourReunioes.map((reuniao) => (
                    ...
                    >
                      <div className="flex items-center gap-1 mb-1">
                        ...
                        <span className="text-xs font-medium">{reuniao.horaInicio} - {reuniao.horaFim}</span>
                      </div>
                ...
                {hourReunioes.map((reuniao) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className={`w-3 h-3 rounded-full ${getPrioridadeColor(reuniao.prioridade)}`}
                           style={{ borderLeftWidth: '4px' }}></div>
                    </div>
                    
                    {reuniao.descricao && (
                      <p className="text-sm opacity-90 mb-2">{reuniao.descricao}</p>
                    )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        reuniao.status === 'em_andamento' ? 'bg-white/20' :
                        reuniao.status === 'agendada' ? 'bg-blue-500' :
                        reuniao.status === 'finalizada' ? 'bg-gray-500' :
                        'bg-red-500'
                      }`}>
                        {reuniao.status === 'em_andamento' ? 'Em andamento' :
                         reuniao.status === 'agendada' ? 'Agendada' :
                         reuniao.status === 'finalizada' ? 'Finalizada' :
                         reuniao.status === 'cancelada' ? 'Cancelada' : reuniao.status}
                      </span>
                    </div>
                  </div>
                ))}
