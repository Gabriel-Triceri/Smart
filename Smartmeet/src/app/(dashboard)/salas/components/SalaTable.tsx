...
import { Edit, Trash2, Building2, Users, CheckCircle, XCircle, Clock } from "lucide-react"
              ...
              {salas.map((sala) => (
                <tr key={sala.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900 max-w-xs truncate" title={sala.recursos}>
                      {sala.recursos || "Nenhum recurso"}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(sala.status)}`}
                    >
